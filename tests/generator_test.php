...
class generator_test extends \advanced_testcase {
    ...
    public function test_create_instance() {
        ...
        $this->assertFalse($DB->record_exists('exescorm', array('course' => $course->id)));
        $exescorm = $this->getDataGenerator()->create_module('exescorm', array('course' => $course));
        $records = $DB->get_records('exescorm', array('course' => $course->id), 'id');
        $this->assertEquals(1, count($records));
        $this->assertTrue(array_key_exists($exescorm->id, $records));

        $params = array('course' => $course->id, 'name' => 'Another exescorm');
