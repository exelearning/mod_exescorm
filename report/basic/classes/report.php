...
namespace exescormreport_basic;
...
class report extends \mod_exescorm\report {
    ...
    public function display($exescorm, $cm, $course, $download) {
        ...
        // Detailed report.
        $mform = new \mod_exescorm_report_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            $detailedrep = $fromform->detailedrep;
            $pagesize = $fromform->pagesize;
            set_user_preference('exescorm_report_detailed', $detailedrep);
            set_user_preference('exescorm_report_pagesize', $pagesize);
        } else {
            $detailedrep = get_user_preferences('exescorm_report_detailed', false);
